...
void compute_simulated_parameters(){
  ...
  final_tree.Branch("sigma_vis_pic_1MeV_error",&sigma_vis_pic_1MeV_error);
  ...
  std::array<std::array<TH1D*, 42>, 520> histo_evis;
  std::array<std::array<TH1D*, 42>, 520> histo_evis_bcu;
  std::array<std::array<TH1D*, 42>, 520> histo_evis_bc;
  std::array<std::array<TH1D*, 42>, 520> histo_evis_u;
  ...
  for(int i=0; i<520; i++){
    for(int j=0; j<42; j++){
      if(i<520){
	histo_evis[i][j] = new TH1D(Form("om_%d_source_%d",i,j),Form("om_%d_source_%d",i,j),nb_bins,min_charge,max_charge);
	// histo_evis_bcu[i][j] = new TH1D(Form("om_bcu_%d_source_%d",i,j),Form("om_bcu_%d_source_%d",i,j),nb_bins,min_charge,max_charge);
	// histo_evis_bc[i][j] = new TH1D(Form("om_bc_%d_source_%d",i,j),Form("om_bc_%d_source_%d",i,j),nb_bins,min_charge,max_charge);
	// histo_evis_u[i][j] = new TH1D(Form("om_u_%d_source_%d",i,j),Form("om_u_%d_source_%d",i,j),nb_bins,min_charge,max_charge);
      }    
    }
  }
  ...
  for(int entry=0; entry<Result_tree->GetEntries();entry++){
	...
	if(num_om->at(k)>519){continue;}
	  ...
	  histo_evis[num_om->at(k)][num_source->at(k)]->Fill(evis->at(k)*1000);
	  // histo_evis_bcu[num_om->at(k)][num_source->at(k)]->Fill(evis_bcu->at(k)*1000);
	  // histo_evis_bc[num_om->at(k)][num_source->at(k)]->Fill(evis_bc->at(k)*1000);
	  // histo_evis_u[num_om->at(k)][num_source->at(k)]->Fill(evis_u->at(k)*1000);
      }
    }
  }
  ...
  for(int i=0; i<520; i++){
    ...
    nb_entries = histo_evis[i][source]->GetEntries();
    source_number=source;
    if(histo_evis[i][source]->GetEntries()>500 && histo_evis[i][source]->GetMean()>0){
	  ...
	  TSpectrum spectrum;
          if (spectrum.Search(histo_evis[i][source], 10, "", 0.11) == 2){
	  ...
	  if (spectrum.Search(histo_evis[i][source], 10, "", 0.11) == 2){
        ...
        fitResult = histo_evis[i][source]->Fit(f_MultipleGaus, "RQ0");
	histo_evis[i][source]->Fit(f_MultipleGaus_pic1, "RQ0+");
	// histo_evis_bcu[i][source]->Fit(f_MultipleGaus_pic1, "RQ0+");
	// histo_evis_bc[i][source]->Fit(f_MultipleGaus_pic1, "RQ0+");
	// histo_evis_u[i][source]->Fit(f_MultipleGaus_pic1, "RQ0+");

	histo_evis[i][source]->Fit(f_MultipleGaus, "RQ0+");	
	// histo_evis_bcu[i][source]->Fit(f_MultipleGaus, "RQ0+");
	// histo_evis_bc[i][source]->Fit(f_MultipleGaus, "RQ0+");
	// histo_evis_u[i][source]->Fit(f_MultipleGaus, "RQ0+");
      }
	...
	// TF1*func = histo_evis_bcu[i][source]->GetFunction("f_MultipleGaus_pic1");
	// TF1*func_pic = histo_evis_bcu[i][source]->GetFunction("f_MultipleGaus");
	// if(func && func_pic){
	//   TCanvas* canvas = new TCanvas(Form("om_bcu_%d",i), Form("om_bcu_%d",i), 800, 600);
	//   canvas->cd();
	//   gStyle->SetOptFit(1111111);
	//   histo_evis_bcu[i][source]->Draw();
	//   func->Draw("same");
	//   func_pic->Draw("same");
	//   canvas->Write();
	//   canvas->Close();
	//   bcu_pic_1MeV = histo_evis_bcu[i][source]->GetFunction("f_MultipleGaus")->GetParameter(1);
	//   bcu_pic_1MeV_error = histo_evis_bcu[i][source]->GetFunction("f_MultipleGaus")->GetParError(1);
	//   bcu_pic_0_5MeV =  histo_evis_bcu[i][source]->GetFunction("f_MultipleGaus_pic1")->GetParameter(1);
	//   bcu_pic_0_5MeV_error =  histo_evis_bcu[i][source]->GetFunction("f_MultipleGaus_pic1")->GetParError(1);
	//   sigma_bcu_pic_1MeV = histo_evis_bcu[i][source]->GetFunction("f_MultipleGaus")->GetParameter(2); 
	// }
	// TF1*func_bc = histo_evis_bc[i][source]->GetFunction("f_MultipleGaus_pic1");
	// TF1*func_bc_pic = histo_evis_bc[i][source]->GetFunction("f_MultipleGaus");
	// if(func_bc && func_bc_pic){
	//   TCanvas* canvas = new TCanvas(Form("om_bc_%d",i), Form("om_bc_%d",i), 800, 600);
	//   canvas->cd();
	//   gStyle->SetOptFit(1111111);
	//   histo_evis_bc[i][source]->Draw();
	//   func_bc->Draw("same");
	//   func_bc_pic->Draw("same");
	//   canvas->Write();
	//   canvas->Close();	  
	//   bc_pic_1MeV = histo_evis_bc[i][source]->GetFunction("f_MultipleGaus")->GetParameter(1);
	//   bc_pic_1MeV_error = histo_evis_bc[i][source]->GetFunction("f_MultipleGaus")->GetParError(1);
	//   bc_pic_0_5MeV =  histo_evis_bc[i][source]->GetFunction("f_MultipleGaus_pic1")->GetParameter(1);
	//   bc_pic_0_5MeV_error =  histo_evis_bc[i][source]->GetFunction("f_MultipleGaus_pic1")->GetParError(1);
	// }
	// TF1*func_u = histo_evis_u[i][source]->GetFunction("f_MultipleGaus_pic1");
	// TF1*func_u_pic = histo_evis_u[i][source]->GetFunction("f_MultipleGaus");
	// if(func_u && func_u_pic){
	//   TCanvas* canvas = new TCanvas(Form("om_u_%d",i), Form("om_u_%d",i), 800, 600);
	//   canvas->cd();
	//   gStyle->SetOptFit(1111111);
	//   histo_evis_u[i][source]->Draw();
	//   func_u->Draw("same");
	//   func_u_pic->Draw("same");
	//   canvas->Write();
	//   canvas->Close();

	//   u_pic_1MeV = histo_evis_u[i][source]->GetFunction("f_MultipleGaus")->GetParameter(1);
	//   u_pic_1MeV_error = histo_evis_u[i][source]->GetFunction("f_MultipleGaus")->GetParError(1);
	//   u_pic_0_5MeV =  histo_evis_u[i][source]->GetFunction("f_MultipleGaus_pic1")->GetParameter(1);
	//   u_pic_0_5MeV_error =  histo_evis_u[i][source]->GetFunction("f_MultipleGaus_pic1")->GetParError(1);
	// }
	TF1* func_vis = histo_evis[i][source]->GetFunction("f_MultipleGaus_pic1");
	TF1* func_vis_pic = histo_evis[i][source]->GetFunction("f_MultipleGaus");
	if(func_vis && func_vis_pic){
	  TCanvas* canvas = new TCanvas(Form("om_vis_%d_source_%d",i,source), Form("om_vis_%d_source_%d",i,source), 800, 600);
	  canvas->cd();
	  gStyle->SetOptFit(1111111);
	  histo_evis[i][source]->Draw();
	  func_vis->Draw("same");
	  func_vis_pic->Draw("same");
	  canvas->Write();
	  canvas->Close();
	  sigma_vis_pic_1MeV = histo_evis[i][source]->GetFunction("f_MultipleGaus")->GetParameter(2);
	  sigma_vis_pic_1MeV_error = histo_evis[i][source]->GetFunction("f_MultipleGaus")->GetParError(2);
	  vis_pic_1MeV = histo_evis[i][source]->GetFunction("f_MultipleGaus")->GetParameter(1);
	  vis_pic_1MeV_error = histo_evis[i][source]->GetFunction("f_MultipleGaus")->GetParError(1);
	  vis_pic_0_5MeV =  histo_evis[i][source]->GetFunction("f_MultipleGaus_pic1")->GetParameter(1);
	  vis_pic_0_5MeV_error =  histo_evis[i][source]->GetFunction("f_MultipleGaus_pic1")->GetParError(1);
	}
	time_bi = time_bi;
	final_tree.Fill();
      }
